		...
		for (var i = 0; i < model.length; ++i) {
			...
			for (var j = 0; j < model[i].length; ++j) {
				...
				svgContent += '\n' + genShapeValue(
					...
					config.colors[model[i][j].color]) + '\n';

				// ajout size de l'objet
				svgContent += '\n' + genTextValue(
					model[i][j].size,
					config.margins.left + j * config.object.width + (j * config.margins['hori-objects']),
					config.margins.top + i * config.object.height + (i * config.margins['verti-objects']),
...
function AEengine(model) {
	...
	var shapeCombi = {
		...
		'circle-trapeze': 'whatever',
		'circle-square': 'whatever',
		'rectangle-circle': 'whatever',
		'rectangle-trapeze': 'quadrilateral',
		'rectangle-square': 'rectangle',
		'rectangle-whatever': 'whatever',
		'rectangle-quadrilateral': 'quadrilateral',
		'polygon-triangle': 'polygon',

		'circle-whatever': 'whatever',
		'circle-quadrilateral': 'whatever',
		'circle-parallelogram': 'whatever',
		'circle-triangle': 'whatever',
		'square-whatever': 'whatever',
		'square-quadrilateral': 'quadrilateral',
		'square-parallelogram': 'parallelogram',
		'triangle-whatever': 'whatever',
	...
	function genPrint(sol) {
		var ret = "<span>shape : </span>" + sol.shape +
			", <span>colors : </span>(" + sol.color + "), <span>size : </span>[" +
	...
	function combi(index, ex, obj) {
		solutions[index].inter = fusionInter(solutions[index].inter, model[ex][obj].size);
		solutions[index].color = fusionColor(solutions[index].color, model[ex][obj].color);
		solutions[index].shape = fusionShape(solutions[index].shape, model[ex][obj].shape);
	}
